...
cleaveland_data_numeric<-cleaveland_data_numeric[,c(which(colnames(cleaveland_data_numeric)=="flag"),which(colnames(cleaveland_data_numeric)!="flag"))]
...
lrn = makeLearner("classif.xgboost", nrounds=8)
...
conf_matrix_values[conf_matrix_values[,2]>0.75 & conf_matrix_values[,3]>=0.75 & conf_matrix_values[,4]>=0.75,]
#We'll choose cutoff that gives better sensitivity so that we can predict
#heart disease cases more accurately.
#Cutoff of 0.45 gives 76% Sensitivity, 80% Specificity & 78% Accuracy

#---------------------------------------------------------    
# plot of cutoff values vs confusion matrix values 
plotcutoff(cutoff_range,conf_matrix_values)
########################################################################
predicted_response <- factor(ifelse(test_pred >=0.45, "Yes", "No"))
actual_response <- factor(ifelse((test$flag==1), "Yes", "No"))
test_conf <- caret::confusionMatrix(data=predicted_response, reference=actual_response, positive = "Yes")
test_conf
...
########################################################################################################################
library(catboost)
train_pool <- catboost.load_pool(data = train_tbl[,-14],label=as.numeric(train_tbl$Diagnosis_Heart_Disease))


#test_pool <- catboost.load_pool(data = test,label=test$flag)
test_pool <- catboost.load_pool(data = test_tbl[,-14])
...
model<-catboost.train(train_pool,  NULL,
                      params = list(loss_function = 'Logloss',
                                    iterations = 100, metric_period=10,depth=6))
endtime=Sys.time()
endtime-starttime
...
err <- mean(as.numeric(pred > 0.5) != test_tbl$Diagnosis_Heart_Disease)
print(paste("test-error=", err))
#test-error= 0.152542372881356


test_conf <- caret::confusionMatrix(data=as.factor(prediction), reference=as.factor(test_tbl$Diagnosis_Heart_Disease))
test_conf
# Accuracy : 0.8475
# Sensitivity : 0.9375          
# Specificity : 0.7407
...
test<-test_tbl
test$flag<-test$Diagnosis_Heart_Disease
...
#We'll choose cutoff that gives better sensitivity so that we can predict
#heart disease cases more accurately.
#Cutoff of 0.37 gives 81% Sensitivity, 78% Specificity & 78% Accuracy

#---------------------------------------------------------    
# plot of cutoff values vs confusion matrix values 
plotcutoff(cutoff_range,conf_matrix_values)
########################################################################
predicted_response <- factor(ifelse(test_pred >=0.37, "Yes", "No"))
actual_response <- factor(ifelse((test$flag==1), "Yes", "No"))
test_conf <- caret::confusionMatrix(data=predicted_response, reference=actual_response, positive = "Yes")
test_conf
